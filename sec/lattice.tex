...
\footnotetext{\todo{We use the reverse ordering of what is originally used in STE to make the abstraction-correspondence clear between $\cap$ and $\sqcap$, $\cup$ and $\sqcup$, and $\subseteq$ and $\sqsubseteq$.}}
...
\noindent \todo{We can now adopt the definition of satisfaction from~\cite{chou1999}}, and say that $\hat M$ \textit{satisfies} a trajectory assertion $\hat \TA$, denoted by $\hat M \LatModels \hat \TA$, iff $\Phi_{*}(s) \meet \pi_{\alpha}(s) \sqsubseteq \Consequent(s)$ for all $s \in S$. That $\hat M$ satisfies $\hat \TA$ implies that a concretisation of $\hat \TA$ can also be satisfied by the original, set-based model $M$.
