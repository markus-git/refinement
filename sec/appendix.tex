...
Using the definition of $\cc$, we see that the assumption $\hat p_{1} \cc \hat q_{1}$ implies that $\sqcup \{ \hat p_{1} \} \cc \sqcap \{ \hat q_{1} \}$. As $\hat q_{1} \sqcap (\hat q_{1} \sqcup \hat q_{2}) = \hat q_{1}$, we also have $\sqcup \{ \hat p_{1} \} \cc \sqcap \{ \hat q_{1}, \hat q_{1} \sqcup \hat q_{2} \}$ and thus $\hat p_{1} \cc \hat q_{1} \sqcup \hat q_{2}$. A similar argument can be used to show $\hat p_{2} \cc \hat q_{1} \sqcup \hat q_{2}$. Since both $\hat p_{1}$ and $\hat p_{2}$ are refined by $\hat q_{1} \sqcup \hat q_{2}$, we must have that $\sqcup \{ \hat p_{1}, \hat p_{2} \} \cc \sqcap \{ \hat q_{1} \sqcup \hat q_{2} \}$ and thus $(\hat p_{1} \sqcup \hat p_{2}) \cc \, (\hat q_{1} \sqcup \hat q_{2})$. The joins of two pairs that refine each other are thus a valid refinement as well. We next show that \todo{it is visibly} a valid simulation relation as well.

The first simulation property of $(\hat p_{1} \sqcup p_{2}) \cc \, (q_{1} \sqcup q_{2})$, that $\Out(\hat p_{1} \sqcup \hat p_{2}) \sqsubseteq \gamma(\Out(\hat q_{1} \sqcup \hat q_{2}))$, is shown in two steps. First, we show that $\gamma(\Out(\hat q_{2})) \sqsubseteq \gamma(\Out(\hat q_{1} \sqcup \hat q_{2}))$ is an upper bound for $\gamma(\Out(\hat q_{1}))$ and $\gamma(\Out(\hat q_{2}))$. Secondly, that $\Out(\hat p_{1} \sqcup \hat p_{2})$ is less than or equal to any other output that is an upper bound on $\hat p_{1}$ and $\hat p_{2}$.
...
\noindent Combining (1) and (2) with the assumption that $\Out(\hat p_{1}) \sqsubseteq \gamma(\Out(\hat q_{1}))$, it follows immediately that $\Out(\hat p_{1} \sqcup \hat p_{2}) \sqsubseteq \gamma(\Out(\hat q_{1} \sqcup \hat q_{2}))$.
...
\todo{Let $\cc \, \subseteq \hat P \times \hat Q$, such that for all $\hat P' \subseteq \hat P$ and $\hat Q' \subseteq \hat Q$}:
