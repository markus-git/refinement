...
\begin{lemma}
Let $S' \subseteq S$. If $\hat M \cc \hat N$ and $\hat p(s) \cc \hat q(s)$ for all $s \in R$, then it must be that $\sqcup \{ F(s)(\hat p(s)) \mid s \in S' \} \cc \sqcup \{ G(s)(\hat q(s)) \mid s \in S' \}$.
\end{lemma}

%% By lemma \todo{?}, and the assumption that $\hat p(s) \cc \hat q(s)$, we know that $F(s)(\hat p(s)) \enc G(s)(\hat q(s))$ for all $s \in R$.

Using induction on the size of $S'$, we show that the join of sets with such pairwise refined predicates also produces two predicates that also refine each other. In the base case, $| S' | = 0$, that $(\top \in \hat P) \cc (\top \in \hat Q)$ follows from the assumption that $\hat M \cc \hat N$. For the inductive step, $| R | = n$, let $\hat p' = \sqcup \{ F(s)(\hat p(s)) \mid s \in \: S' \}$ and $\hat q' = \sqcup \{ G(s)(\hat q(s)) \mid s \in \: S' \}$. Assume that $\hat p' \cc \hat q'$. We extend $S'$ with $s$. From lemma \todo{?} and our assumption $\forall s \in S' : \hat p(s) \cc \hat q(s)$, we know that $F(s)(\hat p(s)) \cc G(s)(\hat q(s))$. That $\hat p' \sqcup F(s)(\hat p(s)) \cc \hat q' \sqcup G(s)(\hat q(s))$ then follows from lemma \todo{?}.
