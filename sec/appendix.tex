...
By definition of $\lll$ and the monotonicity of $\lEq{\cdot}$, we both have $\hat \alpha(\lEq{\hat p}) \sqsubseteq \lEq{\hat q} \sqsubseteq (\lEq{\hat q \sqcup \hat s})$ and $\hat \alpha(\lEq{\hat r}) \sqsubseteq \lEq{\hat s} \sqsubseteq (\lEq{\hat q \sqcup \hat s})$. \todo{Because join produces the least upper bound}, it follows that $(\lEq{\hat p \sqcup \hat r}) \sqsubseteq \hat \gamma(\lEq{\hat q \sqcup \hat s})$, or $(\hat p \sqcup \hat r) \lll (\hat q \sqcup \hat s)$
...
% This goal is equivalent to $\forall \hat p \in \Phi_{*}(s) : \hat p \in \hat{\gamma}(\lAntecedent(s)) \implies \hat p \in \hat{\gamma}(\lConsequent(s))$. So we assume 

% From lemma~\ref{lem:3} we know that $\Phi_{*}(s) \lll \Psi_{*}(s)$ for all $s \in S$.

% $\lEq{\Phi_{*}(s)} \sqsubseteq \hat{\gamma}(\lEq{\Psi_{*}(s)})$
