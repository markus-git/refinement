...
A Galois connection always relates the two bottoms, $\hat \alpha(\bot) \subseteq \bot$, which implies that $\hat \alpha(\lb \bot \rb) \subseteq \lb \bot \rb$, or $\bot \lll \bot$, since $\lb \cdot \rb$ preserves bottom. As $\hat M$ and $\hat N$ also preserves bottom, it follows that $\hat M(\bot) = \bot \lll \bot = \hat N(\bot)$. $\qedbox$

\begin{lemma} \label{lem:closed-join}
$\hat p \lll \hat q \wedge \hat r \lll \hat s \implies (\hat p \sqcup \hat r) \lll (\hat q \sqcup \hat s)$
\end{lemma}

By definition of $\lll$ and the monotonicity of $\lb \cdot \rb$, we both have $\hat \alpha(\lb \hat p \rb) \sqsubseteq \lb \hat q \rb \sqsubseteq (\lb \hat q \sqcup \hat s \rb)$ and $\hat \alpha(\lb \hat r \rb) \sqsubseteq \lb \hat s \rb \sqsubseteq (\lb \hat q \sqcup \hat s \rb)$. \todo{Because join produces the least upper bound}, it follows that $(\lb \hat p \sqcup \hat r \rb) \sqsubseteq \hat \gamma(\lb \hat q \sqcup \hat s \rb)$, or $(\hat p \sqcup \hat r) \lll (\hat q \sqcup \hat s)$
\\
