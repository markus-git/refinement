...
%Given that $c \in \gamma(\Antecedent(\rho))$, there exists an $\textrm{a} \in \Antecedent(\rho)$ such that $\{ c \} \subseteq \gamma(\{ \textrm{a} \})$, or $\alpha(\{ c \}) \subseteq \{ \textrm{a} \}$. By definition of $\{ c \} \ll \{ d \}$, we also have that $[c] \subseteq \gamma([d])$, which implies that $\{ c \} \subseteq \gamma([d])$, or $\alpha(\{ c \}) \subseteq [d]$. Since $\alpha([c]) \neq \emptyset$, it must be that $\{ \textrm{a} \} \cap [d] \neq \emptyset$, thus $\textrm{a} \sim d$. By the invariance of $\Antecedent$, it follows that $d \in \Antecedent(\rho)$. $\qedbox$
