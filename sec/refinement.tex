...
% Partitioning the set of trajectories by their inputs allow refinement to be equivalently stated as an output equality for each part in the family of trajectories, denoted by $M \Refines_{in} N$:

\begin{equation*}
\forall \delta \in \In[C]^{+} : \forall \tau \in \Traj(M)(\delta) : \exists \upsilon \in \Traj(N)(\delta) : \Out(\tau) = \Out(\upsilon)
\end{equation*}

\noindent This definition is equivalent to the earlier one, and we denote it by $M \Refines_{in} N$. \todo{Text.}
