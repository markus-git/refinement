...
In a similar vein, let the inputs of an abstract predicate $\hat P$ be identified by another mapping, $\In \in \hat P \rightarrow \hat P$. \todo{The set of all possible inputs for $\hat P$ is given by the set $\In(\hat P) = \{ \In(\hat p) \mid \hat p \in \hat P \}$}. \todo{The response of $\hat M$ in state $\hat p$ to an input $\hat \iota \in \In(\hat P)$ is determined by $\hat M(\hat \iota \sqcap \hat p)$}. \todo{Intuitively, if the output of $\hat M$ and $\hat N$ is ordered in each possible state reached by a sequence of inputs when starting from $\top$, then $\hat M$ visually refines $\hat N$. Their internal states could differ at times, but externally they behave the same}.

%% ToDo: Badly written.

%% An \textit{input} and \textit{output} of $\hat P$ are thus predicates $\hat \iota, \hat \omicron \in \hat P$ such that $\hat \iota = \In(\hat \iota)$ and $\hat \omicron = \Out(\hat \omicron)$;
