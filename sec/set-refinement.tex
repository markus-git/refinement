...
Let there be a Galois connection between \todo{predicates over equivalence classes $\pow(\C/\msim)$ and $\pow(\D/\msim)$ ordered by set inclusion}. The usual definition of a Galois connection is in terms of an \textit{abstraction} $\alpha \in \pow(\C/\msim) \rightarrow \pow(\D/\msim)$ and a \textit{concretisation} $\gamma \in \pow(\D/\msim) \rightarrow \pow(\C/\msim)$ function, such that $\alpha(\widetilde{C}) \sqsubseteq \widetilde{D} \iff \widetilde{C} \subseteq \gamma(\widetilde{D})$ for all $\widetilde{C} \in \pow(\C/\msim)$ and $\widetilde{D} \in \pow(\D/\msim)$. \todo{For example, a Galois connection between \dots}
