...
We instead adopt a definition of Galois connections as a binary relation $\ll \, \subseteq \mathcal{P}(C) \times \hat P$, where $p \ll \hat p$ reads as ``$p$ can be approximated as $\hat p$'', such that for all $Q \subseteq \mathcal{P}(C)$ and $\hat Q \subseteq \hat P$:

% Let there be Galois connection $\ll \, \subseteq \mathcal{P}(C) \times \hat P$ relating concrete and abstract predicates, where $p \ll \hat p$ reads as ``$p$ can be approximated as $\hat p$'' and for all $Q \subseteq \mathcal{P}(C)$ and $\hat Q \subseteq \hat P$:

\begin{equation*}
\forall p \in Q : \forall \hat p \in \hat Q : p \ll \hat p \iff \cup Q \ll \sqcap \hat Q
\end{equation*}

% from which we can derive $\ll$ as $p \ll \hat p \iff p \subseteq \gamma(\hat p)$ or $p \ll \hat p \iff \alpha (p) \sqsubseteq \hat p$. 

% Q \ll \hat Q

\noindent Intuitively, $\ll$ is an extension of the partial order $\subseteq$ of $\mathcal{P}(C)$ and $\sqsubseteq$ of $\hat P$ to an ordering between $\mathcal{P}(C)$ and $\hat P$. The standard, functional definition of a Galois connection can be derived from $\ll$ as $\gamma(\hat p) \iff \sqcup \{ p \in \mathcal{P}(C) \mid p \ll \hat p \}$, and vice versa as $p \ll \hat p \iff p \subseteq \gamma(\hat p)$.

% As an illustration of this property, consider how we must have that $\{ \langle 1,1,0 \rangle, \langle 1,0,0 \rangle \} \in \mathbb{B}^{3} \ll \{ \langle 1, \X, 0  \rangle \} \in \mathbb{T}_{\bot}^{3}$ for the ternary abstraction.
