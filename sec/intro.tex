...
In practice, signals in $M$ are typically divided into ``input'' signals and ``output'' or ``internal'' signals. While an input signal is typically controlled by the external environment and thus unconstrained by $M$ itself, non-input signals in $M$ are determined by the circuit topology and functionality. If assume our circuit is the earlier unit-delayed two-input AND gate, for example, we could define its model $\mathcal{M} \in \mathcal{P}(\mathbb{B}^{3}) \rightarrow \mathcal{P}(\mathbb{B}^{3})$ as:
...
\subsubsection{Abstract cricuit model}
