...
\todo{This is equivalent to a DFA formulation~\cite{chou1999}.} For all functions $\Phi \in S \rightarrow \pow(C)$ and states $s \in S$, define $F \in S \rightarrow (\pow(C) \rightarrow \pow(C))$ as follows:
...
$M$ \textit{satisfies} a trajectory assertion $A$, denoted by $M \smodels A$, iff $\forall s \in S : \Phi_{*}(s) \cap \pi_{a}(s) \subseteq \pi_{c}(s)$.
...
$\hat M$ \textit{satisfies} a trajectory assertion $\hat A$\footnotemark, denoted by $\hat M \models \hat A$, iff $\forall s \in S : \hat \Phi_{*}(s) \sqcap \pi_{\alpha}(s) \sqsubseteq \pi_{c}(s)$.
